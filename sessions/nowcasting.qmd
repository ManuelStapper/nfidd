...
The aim of this session is to introduce the concept of *nowcasting*, and see how we can perform a nowcast if we know the underlying delay distribution.
...
We call the *reporting time* the time at which a data point (e.g. a time or day of sypmtom onset or a time or day of hospitalisation) has entered the data set used for some analysis.
...
```{r aggregate}
## create aggregated time series of onsets and reports from cut-off data
df_co <- df_co |>
  transmute(
...
By cutting off at a certain *reporting* date, we cannot include many of the recent symptom onsets that are still to be reported.
We can see that if we plot the final data set alongside the cut-off one:

```{r plot_cut_final}
# create aggregated time-series using the complete outbreak data
final <- df |>
...
final_all_days <- expand_grid(day = seq(0, max(final_onset_ts$day))) |>
  ...
  mutate(cutoff = "final")
# combine with cut-off data and plot
...
We could do this using methods from [the session on biases in delay distributions](sessions/biases-in-delay-distributions.qmd).
...
First we define the proportion reported over time using a delay distribution, again using a lognormal distribution with meanlog 1 and sdlog 0.5:
...
We can now construct some simulated data and use this delay distribution to nowcast the most recent data.
Here we use the same simulation approach as in the [renewal session](R-estimation-and-the-renewal-equation) and apply the `reporting_delay` to the last 15 days of data.
...
-   The `proportion_reported` is the cumulative distribution function of the delay distribution. It gives the probability that a report is made on day 1 or earlier, day 2 or earlier, etc. Note that for days more that 15 days into the past we assume reporting is complete.
...
-   On line 21, we define the generative process for the number of onsets. Here we assume that onsets are independent with each drawn from a normal distribution with mean 5 and SD 20.
:::
...
reported_onset_df |> 
  ...
  labs(title = "Symptom onsets: simple nowcast",
       subtitle = "Actual (bar) and nowcast (median and 90%CrI, shaded)")
```
...
Remember, the prior model for the geometric random walk assumes that onsets are the same as the previous day with statistical noise.
...
reported_onset_df |> 
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5) +
  geom_line(aes(y = median), alpha = 0.5) +
  labs(title = "Symptom onsets: nowcast with geometric random walk",
       subtitle = "Actual (bar) and nowcast (median and 90%CrI, shaded)")
```
...
::: {.callout-note collapse="true"} \## Solution - The nowcast better matches the ultimately observed data.
The geometric random walk allows the model to capture the multiplicative growth in the data and so better capture that current incidence is related to past incidence.
...
# Simulate reported onsets, using onset data and reporting delay PMF
reporting_triangle <- onset_df |>
  filter(day < cutoff) |>
  # for each day of known onsets, expand the matrix of daily reporting probabilities
  mutate(
    reporting_delay = list(tibble(d = 0:15, reporting_delay = reporting_delay_pmf)
  )) |>
  unnest(reporting_delay) |>
  mutate(
    # get onsets reported each day
    reported_onsets = onsets * reporting_delay,
    # add Poisson observation error
    noisy_reported_onsets = rpois(n(), reported_onsets)
  ) |>
...
noisy_onsets_df <- reporting_triangle |>
  group_by(day) |>
  summarise(noisy_onsets = sum(noisy_reported_onsets)) |>
  ungroup()
```
...
noisy_onsets_df |> 
  ...
  labs(title = "Symptom onsets: joint estimation nowcast",
       subtitle = "Actual (bar) and nowcast (median and 90%CrI, shaded)")
```
...
-   In addition, whilst we say it is "quite simple" as should be clear from the code it is actually quite complex and computationally intensive. This is because we are fitting a model to the reporting triangle which is a much larger data set and so the model is relatively quite slow to fit.
:::
...
```{r gt}
plot(gen_time_pmf, main = "Generation time PMF")
```

and

```{r ip}
plot(ip_pmf,  main = "Incubation period PMF")
```
...
-   The key takeway from the reproductio number plot is that it looks similar to the one we estimated in the [renewal session](R-estimation-and-the-renewal-equation). This is because we have accounted for the truncation (otherwise it would be spuriously decreasing towards the end of the timeseries).
:::
