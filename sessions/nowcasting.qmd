...
By cutting off at a certain _reporting_ date, we cannot include many of the recent symptom onsets that are still to be reported.
...
As we saw in the [session on the renewal equation](R-estimation-and-the-renewal-equation), a geometric random walk is a simple way to model multiplicative growth. Remember, the prior model for the geometric random walk assumes that onsets are the same as the previous day with statistical noise. Adding this into our simple nowcasting model may help us to better capture the generative process of the data and so produce a better nowcast.
...
ggplot(joint_rt, aes(x = day, y = median)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5) +
  labs(title = "Reproduction number: joint estimation nowcast")
```
